...
<head>
<meta charset="utf-8">
<title> <?php foreach($articles as $a): ?><?=$a["title"]?><?php endforeach ?></title>
...
<body>
    ...
    <img id="ii"src="img/<?php foreach($articles as $a): ?><?=$a['id']?><?php endforeach ?>.jpg">
    <div class="bg" ><div id="MM"><div id="nm">menu</div></div>
        <h1 id="logo">Подольск <div id="rr"><?php foreach($articles as $a): ?><?=$a['title']?><?php endforeach ?></div></h1>
        <div class="h-l"></div>
    <a href="" title="Cчелкните для перехода к другому объекту"><div id="rrt" >Другое место</div></a>
        
    
        <div class="op">
            <div class="text"><?php foreach($articles as $a): ?><?=$a['content']?><?php endforeach ?></div>
            <img class="photo" id="f1" src="img/<?php foreach($articles as $a): ?><?=$a['id']?><?php endforeach ?>a.jpg">
            <img class="photo" id="f2" src="img/<?php foreach($articles as $a): ?><?=$a['id']?><?php endforeach ?>b.jpg">
            <img class="photo" id="f3" src="img/<?php foreach($articles as $a): ?><?=$a['id']?><?php endforeach ?>c.jpg">
        </div>
        <div class="map">
        
           <?php foreach($articles as $a): ?><?=$a['map']?><?php endforeach ?>


            
        </div>
        <div class="likes">
            <div class="LK" title="Столько людей видилм это место">
...
<script>
    $('.bar').css({"height":window.screen.height});
